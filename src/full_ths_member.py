# -*- coding: utf-8 -*-
"""全量：同花顺概念板块成分（ths_member）
- 代码来源：pro.ths_index(type='N') 只取“概念”板块   (doc_id=259)
- 成分：pro.ths_member(ts_code=...)                     (doc_id=261)
- 规范：按板块逐个“先删后写”，UPSERT 幂等，NaN/NA→None
"""
...
def _get_concept_codes(pro):
    # 仅“概念”板块：type='N'（概念），官方定义见 ths_index 文档
...
def main():
    ...
    log.info(f"[full] table={TABLE} concept_boards={len(boards)}")

    total = 0
    for i, b in enumerate(boards, 1):
        try:
            _delete_one_board(eng, b)            # 逐板块清空后重下
            df = _fetch_members(pro, b)
            n  = _upsert(eng, df)
            total += n
            if n or (i % 50 == 0):
                log.info(f"[full] {i}/{len(boards)} board={b} rows={n} total={total}")
            time.sleep(0.06)
        except Exception as e:
            log.exception(f"[full] {i}/{len(boards)} board={b} error: {e}")
            time.sleep(0.12)
