# -*- coding: utf-8 -*-
"""
批量增量下载：A股每日指标（TuShare daily_basic）
- 自动将 end 对齐最近开市日（周末/节假日自动回退）
- 股票池三层兜底：本地 index_weight 去重 -> 指数快照回退 -> 全市场
- 保持原日志/入库/参数风格
最后更新: 2025-08-31
"""
...
def _normalize(df: pd.DataFrame) -> pd.DataFrame:
    if df is None or df.empty:
        return pd.DataFrame(columns=[
            "ts_code_t","trade_date_t","close_t","turnover_rate_t","turnover_rate_f_t","volume_ratio_t",
            "pe_t","pe_ttm_t","pb_t","ps_t","ps_ttm_t","total_share_t","float_share_t","free_share_t",
    ...
    df.rename(columns={
        "ts_code":"ts_code_t","trade_date":"trade_date_t","close":"close_t",
        "turnover_rate":"turnover_rate_t","turnover_rate_f":"turnover_rate_f_t",
        "volume_ratio":"volume_ratio_t","pe":"pe_t","pe_ttm":"pe_ttm_t",
        "pb":"pb_t","ps":"ps_t","ps_ttm":"ps_ttm_t","total_share":"total_share_t",
        "float_share":"float_share_t","free_share":"free_share_t",
        "total_mv":"total_mv_t","circ_mv":"circ_mv_t"
    }, inplace=True)
...
def main(sleep_s: float, init_start: str, index_code: str, align_open_day: int):
    ...
    for i, code in enumerate(codes, 1):
        try:
            df = pro.daily_basic(ts_code=code, start_date=start, end_date=end, fields=FIELDS)
