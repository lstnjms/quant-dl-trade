# -*- coding: utf-8 -*-
"""
批量全量下载：A股复权因子（TuShare adj_factor）
- 支持全市场或指定指数成分股
"""
...
def _normalize(df: pd.DataFrame) -> pd.DataFrame:
    if df is None or df.empty:
        return pd.DataFrame(columns=["ts_code_t","trade_date_t","adj_factor_t"])
    df = df.copy()
    ...
    df["adj_factor_t"] = pd.to_numeric(df["adj_factor_t"], errors="coerce")
    return df[["ts_code_t","trade_date_t","adj_factor_t"]]

def _upsert(engine, df: pd.DataFrame) -> int:
    if df is None or df.empty: return 0
    cols = ["ts_code_t","trade_date_t","adj_factor_t"]
    df = df[cols].copy()
...
def main(start: str, end: str, sleep_s: float, index_code: str):
    ...
    for i, code in enumerate(codes, 1):
        try:
            df = pro.adj_factor(ts_code=code, start_date=start, end_date=end, fields=FIELDS)
