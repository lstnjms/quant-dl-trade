...
from utils import get_engine, get_pro as utils_get_pro, log, today_str, delete_range
...
DEFAULT_QPM   = 480.0
BADCSV        = "bad_nineturn_full.csv"

# ---------- token/Pro ----------
def _get_pro_safe():
    """优先用 utils.get_pro()；失败则用 config.TUSHARE_TOKEN 强制创建"""
    try:
        pro = utils_get_pro()
        # 简单探测：能跑就用
...
def main(start: str, end: str, index: str, qpm: float):
    ...
    # 幂等清区间
    delete_range(eng, TABLE, DATECOL, _to_date_only(start), _to_date_only(end))
    ...
    for i, code in enumerate(codes, 1):
        ...
        try:
            ...
            if df is None or df.empty:
                if i % 100 == 0: log.info(f"[full] {i}/{len(codes)} {code} empty")
                continue
            ...
            log.warning(f"[full] {i}/{len(codes)} {code} err: {e}")

    log.info(f"[full] done table={TABLE} rows={total} range={start}~{end} index={index or 'ALL'}")
