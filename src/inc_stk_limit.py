# -*- coding: utf-8 -*-
"""
增量下载：每日涨跌停价格（TuShare stk_limit）
- 从库内 MAX(trade_date_t) 的下一交易日起补到今天
- 新增：--index 指数代码（如 000905.SH），仅保留该指数历史成分股
- 新增：--qpm 每分钟请求上限（默认 480）
"""
...
def main(index_code: str, qpm: float, init_start: str):
    pro = get_pro()
    eng = get_engine()
    ...
    start = last if last else init_start

    days = _trade_days(pro, start, end)
    if not days:
        log.info(f"[inc] table={TABLE} no trade days {start}~{end}")
        return
    # 若 start 恰为库内最后交易日，则从下一个交易日开始
    if last and last in days:
        days = days[days.index(last)+1:]

    member_set = _get_member_set(pro, index_code.strip() or None, start, end)
    log.info(f"[inc] table={TABLE} index={index_code or 'ALL'} qpm={qpm} trade_days={len(days)} range={start}~{end}")
    ...
    for i, ymd in enumerate(days, 1):
        try:
            ...
            if df.empty:
                if i % 20 == 0: log.info(f"[inc] {i}/{len(days)} {ymd} empty")
                continue
            n = _upsert(eng, df)
            total += n
            if n or (i % 20 == 0):
                log.info(f"[inc] {i}/{len(days)} {ymd} rows={n} total={total}")
        except Exception as e:
            log.warning(f"[inc] {i}/{len(days)} {ymd} error: {e}")
            time.sleep(0.2)

    log.info(f"[inc] done table={TABLE} rows={total} range={start}~{end}")
...
if __name__ == "__main__":
    ...
    args = ap.parse_args()
    main(args.index.strip(), args.qpm, args.init_start.strip())
