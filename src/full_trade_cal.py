# -*- coding: utf-8 -*-
"""全量下载：交易日历 trade_cal（仅全市场；先删区间/剔除SSE/SZSE/BSE再重下）
- 接口：pro.trade_cal（https://tushare.pro/document/2?doc_id=26）
- 表：trade_cal（主键：exchange_t + cal_date_t）
- 说明：默认只抓取 全市场('')；如需指定可用 --exchanges（例："SSE" 或 "SSE,SZSE"）
"""
...
def _delete_range(engine, exchanges: List[str], start: str, end: str):
    """删除旧数据：
       - 若仅抓全市场('')：先全表彻底清掉 SSE/SZSE/BSE，再删除全市场在区间内的旧记录；
       - 若手动指定了其它交易所：仅删对应交易所在区间的记录。
    """
    s, e = _to_date(start), _to_date(end)
    # 仅抓全市场：额外做一次“剔除沪深北”的全表清理
    if exchanges == [""]:
        log.info(f"[purge] table={TABLE} remove SSE/SZSE/BSE entirely")
        purge_sql = (
            f"DELETE FROM `{TABLE}` WHERE `exchange_t` IN ('SSE','SZSE','BSE')"
        )
        with engine.begin() as conn:
            res = conn.execute(text(purge_sql))
            log.info(f"[purge] rows={getattr(res, 'rowcount', -1)}")

        # 仅删除全市场在区间的旧记录
        log.info(f"[full] table={TABLE} delete_range exchange='' {start}~{end}")
        with engine.begin() as conn:
            res = conn.execute(
                text(
                    f"DELETE FROM `{TABLE}` "
                    f"WHERE `{DATE_COL}`>=:s AND `{DATE_COL}`<=:e AND `exchange_t`=''"
                ),
                {"s": s, "e": e}
            )
            log.info(f"[delete] rows={getattr(res, 'rowcount', -1)}")
        return

    # 其余情况：仅删指定交易所在区间的记录（保持你原逻辑）
...
if __name__ == "__main__":
    ...
    # 注意：现在的 ALL 表示“仅全市场”，需要抓其它交易所时显式传：--exchanges SSE,SZSE
    ap.add_argument("--exchanges", default="ALL",       help='ALL=仅全市场；或逗号分隔 "SSE,SZSE"；全市场用空串: ""')
    args = ap.parse_args()
