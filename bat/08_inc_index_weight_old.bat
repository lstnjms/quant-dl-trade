...
set "LOG_FILE=%LOG_DIR%\08-inc_index_weight-!TS!.log"

echo [START] 08-指数成分权重(增量) %%date%% %%time%% > "%LOG_FILE%"
call "%CONDA_ROOT%\Scripts\activate.bat" "%CONDA_ENV%" >> "%LOG_FILE%" 2>&1
pushd "%WORK_DIR%" >> "%LOG_FILE%" 2>&1

rem === 具体任务 ===
python inc_index_weight.py --index 000905.SH --latest >> "%LOG_FILE%" 2>&1
set "ERR=!ERRORLEVEL!"
